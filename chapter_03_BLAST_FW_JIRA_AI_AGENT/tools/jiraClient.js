...
import { getMockIssue } from './mockIssues.js';
...
export async function fetchIssue(config, jiraId) {
  ...
  const upperId = jiraId.trim().toUpperCase();
  const base = (config.jiraUrl || '').trim().replace(/\/+$/, '');
  const hasCreds = base && config.jiraEmail && config.jiraToken;

  // Intercept mock keys or when credentials are missing
  if (upperId === 'LOGIN-101' || upperId === 'DASH-202' || !hasCreds) {
    console.log(`[jiraClient] Fetching mock issue for key: ${upperId}`);
    return getMockIssue(upperId);
  }
